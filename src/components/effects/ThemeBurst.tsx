...
/** Three CSS keyframe layers (`burst-ink`, `burst-flash`, `burst-ring`). */
function BurstLayer({ burst }: { burst: BurstEvent }) {
  ...
  return (
    <>
      ...
      <div
        className="burst-ink absolute rounded-full"
        style={{
          left: burst.x,
          top: burst.y,
          width: 24,
      ...
      {/* Full-screen flash */}
      <div
        className="burst-flash absolute inset-0"
        style={{ background: color, "--flash": isDark ? 0.35 : 0.18 } as React.CSSProperties}
      />

      {/* Radial energy ring */}
      <div
        className="burst-ring absolute rounded-full border-2"
        style={{
          left: burst.x,
          top: burst.y,
          width: 40,
